...
- [C++で形式的証明](#cで形式的証明)
  ...
  - [3. 命題論理の実装](#3-命題論理の実装)
    ...
    - [3.9. `Prop`](#39-prop)
    - [3.10. 補足](#310-補足)
      - [3.10.1. コピーコンストラクタについて](#3101-コピーコンストラクタについて)
      - [3.10.2. ラムダ式の副作用について](#3102-ラムダ式の副作用について)
  - [4. 命題論理の証明例](#4-命題論理の証明例)
    - [4.1. 対偶律](#41-対偶律)
    - [4.2. ド・モルガンの法則](#42-ドモルガンの法則)
    - [4.3. 二重否定の導入・除去](#43-二重否定の導入除去)
    - [4.4. 同一律](#44-同一律)
  - [5. 述語論理の公理・推論規則](#5-述語論理の公理推論規則)
  - [6. 述語論理の実装](#6-述語論理の実装)
    - [6.1. 基本方針](#61-基本方針)
    - [6.2. メタ関数](#62-メタ関数)
    - [6.3. 基底クラス](#63-基底クラス)
    ...
    - [6.7. `Bound`](#67-bound)
    - [6.8. 補足](#68-補足)
      - [6.8.1. 命題としての区別について](#681-命題としての区別について)
      - [6.8.2. 束縛変数に重複がある命題について](#682-束縛変数に重複がある命題について)
  - [7. 述語論理の証明例](#7-述語論理の証明例)
    - [7.1. ド・モルガンの法則 (述語論理版)](#71-ドモルガンの法則-述語論理版)
    - [7.2. `Exist` と `All` の順序交換](#72-exist-と-all-の順序交換)
    - [7.3. `Exist` の `And` に対する分配](#73-exist-の-and-に対する分配)
    - [7.4. `All` の `Or` に対する分配](#74-all-の-or-に対する分配)
  - [8. 実装の全体像](#8-実装の全体像)
  - [9. アピールポイント](#9-アピールポイント)

<div style="page-break-before:always"></div>

## 1. 概要

本レポートでは述語論理の形式的証明ツールをC\+\+で実装します。形式的証明のコンパイラをC\+\+で書くという意味ではなく、C\+\+の言語機能をそのまま形式的証明の構文に利用し、C\+\+のコードとして形式的証明をコンパイルすることを目指します。正しい証明に対してコンパイルが通るようにすることはもちろん、常識的な範囲の間違いを含む証明に対してコンパイルエラーを出すことも目標にします。
...
もう一つ問題があります。`Or` の `elim` メンバ関数および `Impl`, `Equiv` のコンストラクタでは、仮定となる命題を直接ラムダ式に渡しています。
```cpp
consteval auto elim(auto f, auto g) const {
    auto rf = f(PropBase::object<P>);
    auto rg = g(PropBase::object<Q>);
...
コンストラクタ `Exist(t, ReplaceType<P, x, t>)` は $\exist$ 導入則を表しています。適当な型 `t` のオブジェクトと `Replace<P, x, t>` のオブジェクトから `Exist<x, P>` のオブジェクトを生成できます。型 `t` をテンプレートとして渡していないのはC\+\+の仕様上コンストラクタはテンプレート引数を明示的に指定できないからです。
...
### 7.3. `Exist` の `And` に対する分配

$\exist x(P(x)\land Q(x))\to\exist xP(x)\land\exist xQ(x)$
```cpp
template <class x> using P = Pred<0, x>;
...
$\forall xP(x)\lor\forall xQ(x)\to\forall x(P(x)\lor Q(x))$
```cpp
template <class x> using P = Pred<0, x>;
template <class x> using Q = Pred<1, x>;
using x = Bound<0>;
...
C\+\+のテンプレート機能は非常に強力ですが、一方でラムダ式はHaskellなどの純粋関数型プログラミング言語に見られるものほど柔軟ではないため、形式的証明の構文に組み入れるのはかなり苦戦しました。しかし、C\+\+のモダンな機能をフル活用し、なんとか述語論理の形式的証明までたどり着くことができました。本レポートを通して、C\+\+のポテンシャルを再確認することができたと思います。